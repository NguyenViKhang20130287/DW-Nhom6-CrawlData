...
public class ProcedureDAO {
    public void transformData(int df_id) {
    ...
    public void insertDataToMart(int df_id) {
        new ConfigurationDAO().insertStatusConfig(df_id, "MART-LOADING");
        String nameFunc = "InsertIntoKetQuaXoSoMart";
        try {
            Connection connection = new DBConnection().getConnection("warehouse");
            CallableStatement callableStatement = connection.prepareCall("{call " + nameFunc + "()}");

            callableStatement.execute();
            System.out.println("Run: " + nameFunc + " success");
            connection.close();
            new ConfigurationDAO().insertStatusConfig(df_id, "MART-LOADED");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
