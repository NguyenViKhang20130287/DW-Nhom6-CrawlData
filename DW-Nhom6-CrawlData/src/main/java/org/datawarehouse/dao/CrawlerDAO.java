...
public class CrawlerDAO {
    ...
    public List<KQXS> resultMTMN(String sourcePath, String dateInput) throws Exception {
        ...
        if (runDate.equals("on")) {
            ...
            //9.1.1set ngày của dateInput là ngày hiện tại
        } else if (runDate.equals("off")) {
            for (String region : regions) {
...
//        }
    ...
    public void exportFileExcel(int configId, String location, String sourcePath, String fileName, String date) {
        ...
        try {
            ...
            //  Tồn tại ?
            if (!file.exists()) {
                // 9.4 Insert thêm dòng mới có status = CRAWLING trong control.data_files
                ...
                //9.8 Insert thêm dòng mới có status = CRAWLED trong control.data_files
             ...
             // 9.4 Insert thêm dòng mới có status = CRAWLING trong control.data_files
                ...
                //9.6 Tạo các cột để lưu dữ liệu trong file Excel
                int rowNum = 1;
                for (KQXS kq : kqxs) {
                    Row row = sheet.createRow(rowNum++);
                    row.createCell(0).setCellValue(kq.getRegion());
                ...
                // 9.7 Insert dữ liệu từ nguồn vào Excel
                workbook.write(fos);
