CREATE DEFINER=`root`@`localhost` PROCEDURE `insertDataToGiaiDim`()
BEGIN
...
INSERT INTO warehouse.giai_dim (warehouse.giai_dim.tenGiai)
SELECT DISTINCT k.giai
FROM staging.ketquaxoso_staging k
WHERE NOT EXISTS (
    SELECT 1
    FROM warehouse.giai_dim m
    WHERE m.tenGiai = k.giai
);
...
IF rowCount > 0 THEN
        ...
        COMMIT;
SELECT 'Dữ liệu đã được thêm vào bảng giai_dim.' AS Message;
ELSE
        -- Rollback giao dịch nếu không có bản ghi nào được thêm
        ROLLBACK;
SELECT 'Không có dữ liệu mới cần thêm vào bảng giai_dim.' AS Message;
END IF;
END